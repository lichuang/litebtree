...
int sqlite3PagerCheckpoint(
  ...
  if( pPager->pWal==0 && pPager->journalMode==PAGER_JOURNALMODE_WAL ){
    /* btreelite: the zero-page WAL edge case that required re-entering the
  ** SQL layer to start a transaction (see SQLite forum fd0f19d229156939)
  ** cannot occur here; btreelite_checkpoint() opens the WAL explicitly
  ** via sqlite3PagerOpenWal() before handing off. */
  }
  if( pPager->pWal ){
